...
def extract_openweather_data(**kwargs):
    ...
    #xcom will store the fecthed data in intermediate memeory of airflow. t1 is the conext variable that we passed.
...
upload_to_s3 = S3CreateObjectOperator(
        ...
        data="{{ ti.xcom_pull(key='final_data') }}",#pulling the data from xcomsss
        dag=dag,
    )

trigger_transform_redshift_dag = TriggerDagRunOperator(#to trigger the other DAG from this DAG
